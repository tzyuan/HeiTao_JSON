			...
			for (var j = 0; j < fileNames.length; j++) {
				...
				var request = {
					...
					url: (pattern.url ? pattern.url.replace(/\$\{fileName\}/g, fileName) : fileName),
                                        fileName:pattern.url.replace(/\$\{fileName\}/g, fileName),
	...
	function loadResource(request, options) {
		...
		if (isStyleSheet(request.contentType)) {
			...
			if (isJavaScript(request.contentType)) {
				...
				head.insertBeforse(element, head.firstChild);
			}
		}
		markRequestLoaded(request);
	}
...
//                        loadResource(request, options);
                        ...
                        if(attrs==''){
                            ...
                            if (isStyleSheet(request.contentType)) {
//                                css=css.length>0?','+request.url1:request.url1;
                                css_arr.push(request.url1);
                            } else {
                                if (isJavaScript(request.contentType)) {
                                    js_arr.push(request.url1);
//                                    jstemp+=jstemp.length>0?','+request.url1:request.url1;
                                }
                            }
                            if(request.purl){
                                purl=request.purl;
                ...
                if(purl!=''){
//                    console.log("<script type=\"text/javascript\" src=\"" + purl+css + "\"></script>");
//                    console.log("<script type=\"text/javascript\" src=\"" + purl+js + "\"></script>");
                    
                    var chunk1=array_chunk(css_arr,10);
                    for(var i=0;i<chunk1.length;i++){
                        document.writeln("<link rel=\"stylesheet\" type=\"text/css\" href=\"" + purl+chunk1[i].join(",") + "\" />");
                    } 
                    
                    var chunk=array_chunk(js_arr,10);
                    for(var i=0;i<chunk.length;i++){
                        document.writeln("<script type=\"text/javascript\" charset=\"UTF-8\" src=\"" + purl+chunk[i].join(",") + "\"></script>");
                    } 
                }
//                
	}
...
(function() {
	...
	for (var i = 0; i < scripts.length; i++) {
		var src = scripts[i].src,
			index = src.indexOf("dorado/boot1.js");
		if (index > -1) {
