...
(function () {
    ...
    dorado.widget.blockview.ImageBlockRenderer = $extend(dorado.widget.blockview.DefaultBlockRenderer, {captionProperty:"caption", imageProperty:"image", tipProperty:null, blankImage:">dorado/client/resources/blank.gif", padding:2, spacing:2, labelHeight:17, stretchImage:false, constructor:function (options) {
        dorado.Object.apply(this, options);
    }, getImageDom:function (dom) {
        var img = dom.firstChild;
        if (img == null) {
            img = $DomUtils.xCreate({tagName:"IMG", style:{position:"absolute"}});
            dom.appendChild(img);
        }
        return img;
        ...
        if (label == null || label.nodeName != "CENTER") {
            label = $DomUtils.xCreate({tagName:"CENTER", style:{position:"absolute"}, content:"^LABEL"});
    ...
    dorado.widget.blockview.TemplateBlockRenderer = $extend(dorado.widget.blockview.DefaultBlockRenderer, {template:"Template", constructor:function (options) {
        dorado.Object.apply(this, options);
    }, render:function (dom, arg) {
    ...
    dorado.widget.AbstractBlockView = $extend(dorado.widget.ViewPortList, {$className:"dorado.widget.AbstractBlockView", ATTRIBUTES:{className:{defaultValue:"d-block-view"}, blockLayout:{writeBeforeReady:true, defaultValue:"vertical"}, lineSize:{}, blockWidth:{defaultValue:80}, blockHeight:{defaultValue:80}, fillLine:{}, renderer:{setter:function (value) {
        if (typeof value == "string") {
            value = eval("new " + value + "()");
        }
        this._renderer = value;
    }}, horiSpacing:{defaultValue:8}, vertSpacing:{defaultValue:8}, horiPadding:{defaultValue:8}, vertPadding:{defaultValue:8}, blockDecoratorSize:{defaultValue:4}, highlightSelectedBlock:{defaultValue:true}}, EVENTS:{onRenderBlock:{}, onBlockMouseDown:{}, onBlockMouseUp:{}, onBlockClick:{}, onBlockDoubleClick:{}, onBlockTap:{}, onBlockTapHold:{}}, createItemModel:function () {
        return new dorado.widget.blockview.BlockItemModel();
    }, createDom:function () {
        var dom = $DomUtils.xCreate({tagName:"DIV", content:{tagName:"DIV", style:{width:"100%", height:"100%"}, content:"^DIV"}});
        var blockView = this;
        ...
        if (dorado.Browser.isTouch || $setting["common.simulateTouch"]) {
            ...
            $fly(dom).bind("tap", function (evt) {
                ...
                if (blockDom) {
                    var data = $fly(blockDom).data("item");
                    if (data) {
                        blockView.fireEvent("onBlockTapHold", blockView, {data:data});
                    }
                }
            });
        } else {
        ...
        var blockDom = $DomUtils.xCreate({tagName:"DIV", className:"block", style:{position:"absolute", overflow:"hidden"}});
        ...
        if (!decorator) {
            decorator = $DomUtils.xCreate({tagName:"DIV", className:"block-decorator block-decorator-hover"});
        ...
        if (itemDom && this._currentBlock != itemDom) {
            $fly(decorator).outerWidth(itemDom.offsetWidth + this._blockDecoratorSize * 2).outerHeight(itemDom.offsetHeight + this._blockDecoratorSize * 2).css({left:itemDom.offsetLeft - this._blockDecoratorSize, top:itemDom.offsetTop - this._blockDecoratorSize}).show();
        } else {
            $fly(decorator).hide();
        }
        ...
        if (!decorator) {
            decorator = $DomUtils.xCreate({tagName:"DIV", className:"block-decorator block-decorator-current"});
            this._container.insertBefore(decorator, this._container.firstChild);
            ...
            if (itemIndex >= this.startIndex && itemIndex <= (this.startIndex + this.itemDomCount)) {
                if (this._blockLayout == "vertical") {
                    if (itemDom.offsetTop < scroller.scrollTop) {
                        ...
                        if ((itemDom.offsetTop + itemDom.offsetHeight) > (scroller.scrollTop + scroller.clientHeight)) {
                            scroller.scrollTop = this._scrollTop = itemDom.offsetTop + itemDom.offsetHeight - scroller.clientHeight + this._vertSpacing;
                        }
                    }
                } else {
                    if (itemDom.offsetLeft < scroller.scrollLeft) {
                        ...
                        if ((itemDom.offsetLeft + itemDom.offsetWidth) > (scroller.scrollLeft + scroller.clientWidth)) {
                            scroller.scrollLeft = this._scrollLeft = itemDom.offsetLeft + itemDom.offsetWidth - scroller.clientWidth + this._horiSpacing;
                        }
                    }
                }
                return;
        ...
        if (itemIndex < this.startIndex) {
            if (this._blockLayout == "vertical") {
                scroller.scrollTop = this._scrollTop = lineIndex * (this._blockHeight + this._vertSpacing) + this._vertPadding - this._vertSpacing;
            } else {
                scroller.scrollLeft = this._scrollLeft = lineIndex * (this._blockWidth + this._horiSpacing) + this._horiPadding - this._horiSpacing;
            }
        } else {
            if (itemIndex > (this.startIndex + this.itemDomCount - 1)) {
                if (this._blockLayout == "vertical") {
                    scroller.scrollTop = this._scrollTop = (lineIndex + 1) * (this._blockHeight + this._vertSpacing) + this._vertPadding - scroller.clientHeight;
                } else {
                    scroller.scrollLeft = this._scrollLeft = (lineIndex + 1) * (this._blockWidth + this._horiSpacing) + this._horiPadding - scroller.clientWidth;
                }
            }
        }
        ...
        if (this._dragMode != "control") {
            ...
            if (itemDom) {
                var contentDom = $DomUtils.xCreate({tagName:"div", className:"d-block-view-dragging-item"});
    ...
    dorado.widget.blockview.getDraggingInsertIndicator = function (direction) {
        ...
        if (indicator == null) {
            indicator = $DomUtils.xCreate({tagName:"div", className:"d-block-view-dragging-insert-indicator-" + code});
            draggingIndicatorHolder["_draggingInsertIndicator-" + code] = indicator;
        }
        return indicator;
    };
})();
dorado.widget.BlockView = $extend(dorado.widget.AbstractBlockView, {$className:"dorado.widget.BlockView", ATTRIBUTES:{currentIndex:{skipRefresh:true, setter:function (index) {
...
dorado.widget.DataBlockView = $extend([dorado.widget.AbstractBlockView, dorado.widget.DataControl], {$className:"dorado.widget.DataBlockView", ATTRIBUTES:{selection:{setter:function (selection) {
    this.refresh();
    $invokeSuper.call(this, arguments);
}}}, getCurrentItem:function () {
